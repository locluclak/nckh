...
def run():
    ...
    SELECTION_F, r= FS.fixedSelection(Y, X,1)
    ...
    for step in range(1,k+1):
        ...
        for i in range(n_fea):
            if (i not in M_hat[:step]):
                ...
                coef = np.linalg.pinv(Xtemp.T @ Xtemp) @ Xtemp.T @ Y
                absRSS = FS.RSS(Y, Xtemp, coef)
                b.append(absRSS)
                A.append(rss_row)

                # print("A:",rss0, "B:", absRSS)
